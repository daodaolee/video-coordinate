...
const VideoCompare: React.FC = () => {
  ...
  const handleImportUrls = async () => {
    ...
    // 去重，排除已存在的url
    const existing = new Set(videos.filter((v) => v.url.startsWith('http')).map((v) => v.url));
    const newUrls = urls.filter((u) => !existing.has(u));
    // 生成VideoSource
    const newVideos: VideoSource[] = await Promise.all(
      newUrls.map(async (url) => {
        // 只用video标签能获取的信息
        const info = await getUrlVideoInfo(url);
        return {
          id: url,
          url,
          name: url,
  ...
  // 获取URL视频的meta（只用video标签能获取的信息）
  async function getUrlVideoInfo(url: string) {
  ...
  return (
    <div className="relative min-h-screen bg-[#000] flex flex-col justify-between p-8 gap-10">
      {/* 导入区：无视频时上下平分，极简风格 */}
      {videos.length === 0 && (
        <div className="w-[calc(100% - 200px)] h-[80vh] flex justify-center items-center">
          <div className="flex flex-col w-[50vw] h-[40vh] rounded-lg overflow-hidden ">
            {/* 上半部分：本地文件导入，极简，50%高度 */}
            <div
              className="flex-1 flex flex-col items-center justify-center cursor-pointer select-none bg-transparent border border-[#333] border-b-0 transition relative group"
              style={{ minHeight: 0 }}
            ...
            >
              <div className="flex flex-col items-center justify-center w-full h-full">
                <FolderOpen
                  size={48}
                  className="text-zinc-600 mb-2 group-hover:text-[#38bdf8] transition"
                />
                <div className="text-zinc-400 text-base mb-1">点击或拖拽导入本地视频</div>
                <div className="text-zinc-500 text-xs">支持多选</div>
                <input
                  type="file"
                  accept="video/*"
                  multiple
                ...
                />
                {dragActive && (
                  <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/40 pointer-events-none rounded-none">
            ...
            {/* 分割线 */}
            <div className="w-full h-0 border-b border-[#333]" />
            {/* 下半部分：URL导入，极简，50%高度 */}
            <div className="flex-1 flex flex-col items-center justify-center bg-transparent border-0">
              <div className="w-full flex flex-row items-center gap-2 mb-1"></div>
              <Textarea
                className="w-full min-h-[120px] bg-transparent text-zinc-100 border border-[#333] placeholder:text-zinc-500 "
                placeholder="粘贴或输入多个视频URL，回车分行"
                value={urlInput}
                onChange={(e) => setUrlInput(e.target.value)}
                disabled={urlLoading}
                onKeyDown={(e) => {
                  if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') handleImportUrls();
                }}
              />
              <Button
                className="bg-[#18181b] text-white border border-[#333] hover:bg-[#333] hover:text-primary mt-10"
      ...
      {videos.length > 0 && (
        <div className="flex flex-row gap-x-2 w-full items-start justify-start overflow-x-auto max-w-[100vw] mb-8">
          {videos.map((video, idx) => {
            ...
            let durationStr = '';
            const durationNum =
              ref?.duration && !isNaN(ref.duration) && ref.duration > 0 ? ref.duration : undefined;
                        ...
                        {video.url.startsWith('http')
                          ? (() => {
                              ...
                              if (meta && typeof meta === 'object' && !Array.isArray(meta)) {
                                const m = meta as UrlVideoMeta;
                                if (!m.error) {
                                  return (
                                    <div className="flex gap-6 w-full">
                                      <div className="flex-1 min-w-0 mb-2 md:mb-0 font-semibold text-zinc-300 flex">
                                        <div className="flex-1">
                                          时长：{m.duration ? formatTime(m.duration) : '未知'}
                                        </div>
                                        <div className="flex-1">
                                          分辨率：
                                          {m.width && m.height
                                        ...
                                        <div className="flex-1">
                                          文件大小：
                                          {m.fileSize
                                            ? `${(Number(m.fileSize) / 1024 / 1024).toFixed(2)} MB`
                                            : '未知'}
                                        </div>
                                      </div>
                                    </div>
                                  ...
                                  return <div className="text-zinc-400">加载失败</div>;
                                }
                              } else {
                                return <div className="text-zinc-400">加载失败</div>;
                              }
